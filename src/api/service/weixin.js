...
module.exports = class extends think.Service {
  async login(code, fullUserInfo) {
    ...
    try {
      ...
      const options = {
        method: 'GET',
        url: `https://api.weixin.qq.com/sns/jscode2session?appid=${think.config('weixin.appid')}&secret=${think.config('weixin.secret')}&js_code=${code}&grant_type=authorization_code`
      };
